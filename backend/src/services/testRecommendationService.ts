...
`;
  ...
  private static createTemplateBasedTestCaseByType(
    testCaseType: 'active' | 'inactive' | 'not_found',
    index: number,
    payerInfo: PayerInfo,
    testCaseId: string
  ): TestCase | null {
   ...
   */
  private static determineTestCaseType(testCaseId: string): 'active' | 'inactive' | 'not_found' {
    const id = testCaseId.toLowerCase();

    // Map to Test Case 2: Inactive Member template
    if (id.includes('inactive') || id.includes('terminated') || id.includes('expired') ||
        id.includes('coverage verification') || id.includes('tc_002')) {
      return 'inactive';
    }
    // Map to Test Case 3: Member Not Found template
    ...
    // Map to Test Case 1: Active Member template (default)
    else {
      return 'active';
    }
  }
