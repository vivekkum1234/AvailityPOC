...
const jsonExportService_1 = require("../services/jsonExportService");
...
router.post('/submit', async (req, res) => {
    try {
        ...
        const organization = await supabaseService_1.supabaseService.createOrganization({
        ...
        const savedResponse = await supabaseService_1.supabaseService.createQuestionnaireResponse(questionnaireResponse);
        const submittedResponse = await supabaseService_1.supabaseService.submitQuestionnaire(savedResponse.id, submittedBy);
        await supabaseService_1.supabaseService.createAuditEntry({
...
router.get('/submission/:id', async (req, res) => {
    try {
        ...
        const submission = await supabaseService_1.supabaseService.getQuestionnaireResponse(id);
        if (!submission) {
            res.status(404).json({
                success: false,
...
router.get('/submissions', async (req, res) => {
    try {
        ...
        if (implementation_mode)
            filters.implementation_mode = implementation_mode;
        const submissions = await supabaseService_1.supabaseService.getQuestionnaireResponses(filters);
        res.json({
            success: true,
            data: submissions,
...
router.post('/draft', async (req, res) => {
    try {
        const { organizationInfo, questionnaireData, implementationMode, responseId, submittedBy, submittedByName } = req.body;
        let organization;
        let response;
        if (responseId) {
            const existingResponse = await supabaseService_1.supabaseService.getQuestionnaireResponse(responseId);
            if (!existingResponse) {
                res.status(404).json({
                    success: false,
            ...
            response = await supabaseService_1.supabaseService.updateQuestionnaireResponse(responseId, updates);
        }
        else {
            if (organizationInfo?.name) {
                organization = await supabaseService_1.supabaseService.createOrganization(organizationInfo);
            }
            const draftResponse = {
                organization_id: organization?.id,
                questionnaire_id: 'x12-270-271-complete',
            ...
            response = await supabaseService_1.supabaseService.createQuestionnaireResponse(draftResponse);
        }
        res.json({
            success: true,
            data: {
...
router.get('/submission/:responseId/edit', async (req, res) => {
    try {
        ...
        const submission = await supabaseService_1.supabaseService.getQuestionnaireResponse(responseId);
        if (!submission) {
...
router.put('/submission/:responseId', async (req, res) => {
    try {
        ...
        const existingSubmission = await supabaseService_1.supabaseService.getQuestionnaireResponse(responseId);
        ...
        if (organizationInfo && organizationInfo.name) {
            const organization = await supabaseService_1.supabaseService.createOrUpdateOrganization(organizationInfo);
            organizationId = organization.id;
        }
        const updates = {
...
function calculateSectionCompletion(responses) {
...
router.post('/fix-completion/:id', async (req, res) => {
    try {
        const { id } = req.params;
        if (!id) {
            res.status(400).json({
        ...
        const existingResponse = await supabaseService_1.supabaseService.getQuestionnaireResponse(id);
        if (!existingResponse) {
            res.status(404).json({
                success: false,
        ...
        const updated = await supabaseService_1.supabaseService.updateQuestionnaireResponse(id, {
...
router.get('/submission/:responseId/export-json', async (req, res) => {
    try {
        const { responseId } = req.params;
        if (!responseId) {
            return res.status(400).json({
        ...
        const response = await supabaseService_1.supabaseService.getQuestionnaireResponse(responseId);
        if (!response) {
            return res.status(404).json({
                success: false,
...
router.post('/submission/:responseId/email-json', async (req, res) => {
    try {
        const { responseId } = req.params;
        const { recipients, subject, message, senderName, senderEmail } = req.body;
        if (!responseId) {
            return res.status(400).json({
                success: false,
                error: 'Response ID is required'
        ...
        await emailService_1.EmailService.sendJsonExport(emailRequest);
        return res.json({
            success: true,
